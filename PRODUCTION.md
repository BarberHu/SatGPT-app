...
Keep `AGENT_WORKERS=1` until LangGraph checkpoints and business-layer state have
moved from process memory to a shared Redis/PostgreSQL store. Multiple workers
currently create inconsistent user sessions. PDF exports use the script supplied
in each POST request and do not share a latest-script variable.

The Agent image also includes the water-asset catalog module, its cached catalog,
and `frontend/src/config/layerCatalog.json`; these are required runtime inputs.
